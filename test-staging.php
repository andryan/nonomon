...
$file=fopen('test-staging.csv','w');
//print_r($host);
foreach ($host as $key => $value) {
//  echo "$host $key $value\n";
  if (@$host[$key]['stream_server']) {
    $line = $host[$key]['user_id'].'|'.$host[$key]['loginname'].'|'.$host[$key]['official'].'|'.$host[$key]['pic'].'|'.$host[$key]['avatar'].'|'.$host[$key]['viewers'].'|'.$host[$key]['stream_server']."\n";
    fwrite($file, $line);
    echo $line;
  }
