...
$country=array('Indonesia','Malaysia','Thailand','Vietnam');
#$country=array('Thailand');
$hostTotal=array();
...
#  echo "DEBUG: $host $key $value\n";
  if (@$hostTotal[$key]['stream_server']) {
    $line = $hostTotal[$key]['user_id'].'|'.str_replace('|','-',$hostTotal[$key]['loginname']).'|'.$hostTotal[$key]['official'].'|'.$hostTotal[$key]['pic'].'|'.$hostTotal[$key]['avatar'].'|'.$hostTotal[$key]['viewers'].'|'.$hostTotal[$key]['stream_server'].'|'.$hostTotal[$key]['country']."\n";
    fwrite($file, $line);
    echo $line;
  }
